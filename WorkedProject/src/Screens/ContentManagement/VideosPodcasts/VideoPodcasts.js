...
const EnhancedTableToolbar = (props) => {
    ...
    const filterVideoBtn = ["btn btn-secondary", filterType === 3 ? "selButton" : "UnselButton"]
    const filterPodcastBtn = ["btn btn-secondary", filterType === 2 ? "selButton" : "UnselButton"]
        ...
        <>
            ...
            >
                ...
                <div className='col-2 col-sm-2 col-md-2 col-lg-2 col-xl-10 col-xxl-9 VideoLevelDiv'>
                    ...
                    <button type="button" className={filterVideoBtn.join(' ')} onClick={() => { selectContentType(3) }}>Videos</button>
                    <button type="button" className={filterPodcastBtn.join(' ')} onClick={() => { selectContentType(2) }}>Podcasts</button>
...
const VideoPodcastsTable = (props) => {
    ...
    const handleChange = (contentId, type) => {
        if (permission === "write") {
            navigate(routes.ADD_VIDEO_PODCASTS, { state: { contentId, selection: type } })
        } else {
    ...
    const handleViewContent = (id, data) => {
        ...
        setShowConfirm(true);
        setSelectedContentStatus(data.is_active)
    };
    ...
    const handleChangeOption = (e, id) => {
        if (permission === "write") {
            if (e.value === "Active") {
                ...
                setSelectedContentStatus(1);
            } else {
                viewContentsMainData.find(v => v.id === id).is_active = 0;
                setSelectedContentStatus(0);
            }
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <div className='imgDivContent'>
                                                            {row.media_thumbnail && <img className="mainIcon" src={row.media_thumbnail} alt="" width="70px" height="70px" />}
                                                    ...
                                                    <TableCell align="left"><p className='othText'>{row.category_name}</p></TableCell>
                                                    <TableCell align="left">
                                                        {row.content_version !== "free" ?
                                                            <div className='contentTypeText'>
                                                                <p className='othText'>{row.content_version}</p>
                                                                <p className='rupeesText'>- ₹{row.amount}</p></div> : <p className='othText'>{row.content_version}</p>}
                                                    </TableCell>
                                                    ...
                                                    <TableCell align="left">
                                                        <ul className="list-group d-flex flex-row ">
                                                            ...
                                                            <li className={row.is_active == 1 ? 'list-group-item ViewEditClick p-0 ' : "list-group-item ViewEditNotClick p-0"}
                                                                onClick={row.is_active == 1 ? () => { handleChange(row.id, "edit") } : ""} ><i className="fa fa-pen mr-1"></i> Edit</li>
            ...
            <Modal centered className="viewContent-modal-warning br-8" show={showConfirm} onHide={() => { setShowConfirm(false) }}>
                ...
                >
                    <Modal.Header>
                        ...
                        <Select
                            className={selectedContentStatus === 1 ? "react-select" : "react-selectChange"}
                            classNamePrefix="react-select"
                            onChange={(e) => { handleChangeOption(e, selectedContentView.id) }}
                            value={selectedContentStatus === 1 ? { label: "Active", value: "Active" } : { label: "Inactive", value: "Inactive" }}
                            options={options}
                            isSearchable={false}
                            theme={(theme) => ({
                                ...theme,
                    ...
                    <Modal.Body>
                        ...
                        <div className="modalData">
                            {selectedContentView.media ?<div className='imgDivContent'>

                                <video className="videoShow" width="600" height="240" controls>
                                    <source src={selectedContentView.media} type="video/mp4" />

                                </video>
                            </div>:<div className='noVideoDiv'>
                                <p className='textNoVideo'>Video not uploaded</p>
                                </div>}
                            <div className='dataContent'>
                                <div className='firstCol'>
                                    <div className='titleTextData'><p className='headeTitle'>Content Type:</p><p className='othText'>{selectedContentView.content_type}</p></div>
                                    <div className='titleTextData'><p className='headeTitle'>Category:</p><p className='othText'>{selectedContentView.category_name}</p></div>
                                </div>
...
export default VideoPodcastsTable;